...
print(os.getcwd())
from yolov5_detect_pallet import Object_detect
...
class Get_coord:
    def __init__(self, rgb, depth):
        ...
        self.grip_pos = None
        self.label = None
        self.detector = None  # Object_detect()
    ...
    def get_pixel_coord(self):
        self.detector = Object_detect()
        self.get_detect_area()
        if self.detect_area is not None:
            self.detector.post_process(self.detect_area)
            if self.detector.area >= 1300 and self.detector.area < 2000:
                print("area:", self.detector.area)
                # print("no result")
                # else:
                ...
                x = int(self.detector.x + self.crop_x)
                y = int(self.detector.y + self.crop_y)
                z = self.depth_data[int((y - 40) / 2), int((x - 40) / 2)]
                self.label = self.detector.label
